...
export default function SavedShow() {
  ...
  useEffect(() => {
    const fetchMovies = async () => {
      ...
      try {
        ...
        const response = await axios.get<{ savedShows: MovieItem[] }>('/api/movies', {
          headers: {
  ...
  const deleteShow = async (favoriteId: number) => {
    try {
      if (!user || !authToken) {
        return;
      }
  ...
  const resolvePosterUrl = (item: MovieItem): string => {
  ...
  const formatReleaseDate = (date: string | undefined): string | null => {
  ...
  const handleNavigate = (movie: MovieItem) => {
