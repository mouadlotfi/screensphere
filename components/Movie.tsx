...
/**
 * Optimized Movie card component with memoization
 * Only re-renders when item, variant, size, or favorite status changes
 */
function MovieComponent({ item, variant = 'slider', size = 'default' }: MovieProps) {
  const router = useRouter();
  const { user, getAuthToken } = useAuth();
  const { isFavorite: checkIsFavorite, getFavoriteId, addFavorite, removeFavorite } = useFavorites();
  const [isLoading, setIsLoading] = useState(false);
  
  const isFavorite = checkIsFavorite(item.id);
  const favoriteId = getFavoriteId(item.id);
  
  // Memoize derived values to prevent recalculation on every render
  const { posterPath, rating, title, posterUrl } = useMemo(() => {
    const path = item?.poster_path || item?.posterPath || item?.backdrop_path || item?.backdropPath || item?.img || item?.image;
    const ratingValue = item?.vote_average ? item.vote_average.toFixed(1) : null;
    const titleValue = item?.title || item?.original_title || item?.name || 'Untitled';
    const url = path
      ? path.startsWith('http')
        ? path
        : `https://image.tmdb.org/t/p/w500${path}`
      : '';
    return { posterPath: path, rating: ratingValue, title: titleValue, posterUrl: url };
  }, [item?.poster_path, item?.posterPath, item?.backdrop_path, item?.backdropPath, item?.img, item?.image, item?.vote_average, item?.title, item?.original_title, item?.name]);
  ...
  }, [user, router, getAuthToken, isLoading, isFavorite, favoriteId, item.id, item.overview, item.release_date, title, posterPath, addFavorite, removeFavorite]);
    ...
    <button type="button" onClick={handleClick} className={containerClasses} aria-label={`View details for ${title}`}>
      <div className="relative aspect-[2/3] w-full overflow-hidden">
        ...
        <div
          ...
          className={`absolute right-2 top-2 z-10 flex h-8 w-8 items-center justify-center rounded-full bg-black/60 backdrop-blur-sm transition-all duration-200 hover:bg-black/80 hover:scale-110 ${
        ...
        >
          {isFavorite ? (
            ...
            <AiOutlineHeart className="h-4 w-4 text-white" />
          )}
        </div>
...
// Only re-renders when props change or when favorite status changes (via context)
const Movie = memo(MovieComponent, (prevProps, nextProps) => {
  return (
