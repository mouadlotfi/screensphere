...
const resolveEndpoint = (genre: string | undefined): string => {
  if (!genre) {
...
export default function GenrePage() {
  ...
  const fetchMovies = useCallback(
    async (pageNumber: number) => {
      try {
        ...
        const response = await axios.get<MoviesResponse>(buildUrl(pageNumber));
  ...
  const heading = useMemo(() => genreTitles[genre as keyof GenreTitles] ?? startCase(genre ?? ''), [genre]);
